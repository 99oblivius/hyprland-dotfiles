...
case "$1" in
    --area)
        ...
        hyprpicker -r -z &
        pid_picker=$!
        trap 'kill -9 "$pid_picker" 2>/dev/null; wait "$pid_picker" 2>/dev/null' EXIT

        # Safety timeout: unfreeze after 10 seconds no matter what
        ( sleep 10; kill -9 "$pid_picker" 2>/dev/null ) &
        pid_timeout=$!
        ...
        region=$(slurp -b "#00000080" -c "#888888ff" -w 1) || {
            kill -9 "$pid_picker" 2>/dev/null
            kill "$pid_timeout" 2>/dev/null
            exit 0
        }
        [[ -z "$region" ]] && { kill -9 "$pid_picker" 2>/dev/null; kill "$pid_timeout" 2>/dev/null; exit 0; }
